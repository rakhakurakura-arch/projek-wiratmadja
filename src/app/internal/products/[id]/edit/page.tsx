...
export default function EditProductPage() {
  ...
  useEffect(() => {
    async function initData() {
      try {
        const [catRes, userRes, prodRes] = await Promise.all([
          fetch('/api/categories'),
          fetch('/api/internal/me'),
          fetch(`/api/internal/products/${productId}`),
        ]);
  ...
  if (!userSession || fetching) return null;
    ...
    <div className="flex min-h-screen bg-ivory-200 text-charcoal-900">
      ...
      <main className="flex-1 p-8 space-y-6 overflow-y-auto max-w-4xl">
        <div className="flex items-center justify-between border-b border-sage-200 pb-4">
          <div>
            <h1 className="font-serif text-3xl font-bold text-charcoal-900">Edit Produk Katalog</h1>
            <p className="text-xs text-sage-600 mt-1">Ubah informasi detail produk dan variannya.</p>
          </div>
          <Link
